...
public class UpgradeVelocityIfStatementsMigrationCheck
	...
	private static void _replaceIfStatementEnd(String[] lines, int i) {
		...
		while (!stack.empty()) {
			...
			if (stack.empty()) {
				lines[j] = StringUtil.replace(
					nextLine, _VELOCITY_END, "</#if>");
			}
		}
	}

	private boolean _hasBreakLine(String newLine) {
		boolean breakLine = false;

		if (StringUtil.count(newLine, CharPool.OPEN_PARENTHESIS) >
				StringUtil.count(newLine, CharPool.CLOSE_PARENTHESIS)) {
	...
	private boolean _isVelocityStatement(String line, String statement) {
		boolean velocityStatement = true;
		...
		if ((line.indexOf(statement) != 0) &&
			...
			velocityStatement = false;
		}

		return velocityStatement;
	}

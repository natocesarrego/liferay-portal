...
public class UpgradeJavaGetLayoutDisplayPageObjectProviderCheck
	...
	protected String beforeFormatIteration(String fileName, String content) {
	...
	protected String format(
		...
		for (JavaTerm childJavaTerm : javaClass.getChildJavaTerms()) {
			...
			javaMethodContent = beforeFormatIteration(
				fileName, javaMethodContent);
			...
			while (matcher.find()) {
				if (!_hasClassNameLong(
						javaMethodContent, content, matcher.group(2)) ||
				...
				Pattern classNamePattern = _getClassNamePattern();

				Matcher classNameMatcher = classNamePattern.matcher(
					javaMethodContent);

				String variableClassName = _WARNING_CASE_VARIABLE;

				if (classNameMatcher.find()) {
					variableClassName = classNameMatcher.group(1);
				}

				String indent = SourceUtil.getIndent(matcher.group(0));

				javaMethodContent = StringUtil.replace(
					javaMethodContent, matcher.group(0),
					_replaceLine(
						variableClassName, matcher.group(2), indent,
						matcher.group(0)));
			}
	...
	private Pattern _getClassNamePattern() {
	...
	private boolean _hasClassNameLong(
		String content, String fileContent, String variableName) {

		return Objects.equals(
			getVariableTypeName(content, fileContent, variableName), "long");
	}

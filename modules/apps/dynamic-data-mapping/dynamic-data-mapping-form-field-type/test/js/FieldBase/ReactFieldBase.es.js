...
describe('ReactFieldBase', () => {
	...
	it('shows the popover when hovering over the tooltip icon', async () => {
		const {container, getByRole, getByTestId, getByText} = render(
			<FieldBaseWithProvider
				popoverHeader="Popover Header"
				popoverImagePath="/forms/image.png"
				showPopover={true}
				spritemap={spritemap}
		...
		expect(getByRole('img')).toHaveAttribute(
			'src',
			'http://localhost:8080/forms/image.png'
		);
		expect(getByRole('img')).toHaveAttribute('width', '232');

		expect(getByText('Popover Header')).toBeInTheDocument();

...
const fieldNameBelongsToAction = (fieldName, actions, pages) => {
	const emptyField = '[]';
...
const findRuleByFieldName = (fieldName, rules, pages) => {
	return rules.some(
		(rule) =>
			fieldNameBelongsToAction(fieldName, rule.actions, pages) ||
			fieldNameBelongsToCondition(fieldName, rule.conditions)
	);
};

const findInvalidRule = (rule, pages) => {
	return findRuleByFieldName('', [rule], pages);
};
